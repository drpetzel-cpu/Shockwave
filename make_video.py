...
def title_frame(t):
    ...
    for i, r in enumerate([260, 390, 520, 650]):
        c = blend(BLUE, 0.04, BG)
        draw.ellipse([W//2-r, H//2-r, W//2+r, H//2+r], outline=c, width=2)
    ...
    if a1 > 0:
        ...
        fc2 = blend(ORANGE, a1)
        draw.arc([W//2-85, fy+40, W//2+200, fy+130], start=0, end=180, fill=fc2, width=7)
        for wi in range(3):
            wc = blend(BLUE, a1 * (0.9 - wi * 0.25))
            draw.arc([W//2-110-wi*30, fy-20-wi*22, W//2-30-wi*30, fy+70-wi*22],
                     start=270, end=90, fill=wc, width=5)

    cx_text(draw, "Shockwave", 540, f(108, bold=True), WHITE, a2)
    cx_text(draw, "Therapy",   662, f(108, bold=True), BLUE,  a2)

    if a3 > 0:
        wrap_text(draw, "The breakthrough solution for Plantar Fasciitis",
                  820, f(52, bold=False), SLATE, W - 140, alpha=a3)
    ...
    if a5 > 0:
        wrap_text(draw, "#ShockwaveTherapy  #PlantarFasciitis  #HeelPain  #FootHealth",
                  1020, f(34, bold=False), SLATE, W - 120, alpha=a5 * 0.7)

    return np.array(img)

title_clip = VideoClip(title_frame, duration=5).with_fps(FPS)

# ── SCENE 2 · PROBLEM (12 s) ──────────────────────────────────────────
...
def problem_frame(t):
    ...
    a_tag  = fade(t, 0,   0.5)
    a_head = fade(t, 0.3, 1.1)
    a_sub  = fade(t, 0.8, 1.6)
    a_div  = fade(t, 1.1, 1.9)
    ...
    wrap_text(draw, "Inflammation of the band of tissue connecting heel to toes",
              480, f(44, False), SLATE, W - 140, alpha=a_sub)

    if a_div > 0:
        dw = int(180 * a_div)
        draw.rounded_rectangle([W//2-dw//2, 582, W//2+dw//2, 587], radius=2,
                                fill=blend(ORANGE, a_div))
    ...
    for i, (title, desc, color) in enumerate(PROB_CARDS):
        delay = 1.8 + i * 1.7
        a = fade(t, delay, delay + 0.55)
        if a <= 0:
            continue
        slide = int((1 - sm(min(1, (t - delay) / 0.55))) * 110)
        cy = card_y + i * (card_h + card_gap)

        draw.rounded_rectangle([60 + slide, cy, W - 60 + slide, cy + card_h],
                                radius=18,
                                fill=blend((18, 36, 65), a),
                                outline=blend(color, a * 0.3), width=1)
        draw.rounded_rectangle([60 + slide, cy, 72 + slide, cy + card_h],
                                radius=9, fill=blend(color, a))

        num_txt = str(i + 1)
        nb = draw.textbbox((0, 0), num_txt, font=f(44, True))
        nw = nb[2] - nb[0]
        draw.text((60 + slide + 42 - nw//2, cy + 30), num_txt,
                  font=f(44, True), fill=blend(color, a))
...
# ── SCENE 3 · SOLUTION (10 s) ──────────────────────────────────────────
BG_S = grad_bg(c1=BG, c2=(10, 32, 68))

SOL_STEPS = [
    ("Acoustic waves reach the heel",
     "Calibrated energy pulses target the inflamed plantar fascia directly"),
    ("Stimulates the body's healing",
     "Triggers blood flow, collagen production & growth factors"),
    ("Breaks down calcium deposits",
     "Dissolves scar tissue driving chronic pain so the body can clear it"),
]
...
def solution_frame(t):
    ...
    if a_anim > 0:
        ...
        pc = blend((40, 100, 170), a_anim)
        draw.rounded_rectangle([W//2-22, py0-100, W//2+22, py0+50], radius=10, fill=pc)
        draw.rounded_rectangle([W//2-28, py0+45, W//2+28, py0+72], radius=8,
                                fill=blend((30, 76, 140), a_anim))
        draw.rounded_rectangle([W//2-14, py0+70, W//2+14, py0+82], radius=4,
                                fill=blend(BLUE, a_anim))
        ...
        for wi in range(3):
            ...
            if wr > 2:
                wc = blend(BLUE, wo, BG)
                draw.arc([W//2 - wr, py0 + 80, W//2 + wr, py0 + 80 + wr * 2],
                         start=15, end=165, fill=wc, width=4)

        fy = py0 + 340
        fc = blend((180, 110, 70), a_anim)
        ...
        fascia_c = blend(ORANGE, a_anim)
        draw.arc([W//2-100, fy+30, W//2+220, fy+120], start=0, end=180, fill=fascia_c, width=7)
    ...
    for i, (title, desc) in enumerate(SOL_STEPS):
        ...
        nt = str(i + 1)
        nb = draw.textbbox((0, 0), nt, font=f(40, True))
        draw.text((cx2-(nb[2]-nb[0])//2, cy2-24), nt, font=f(40, True), fill=blend(WHITE, a))

        if i < len(SOL_STEPS) - 1:
            draw.line([cx2, cy2+cr, cx2, cy2+cr+(step_h-cr*2-10)],
                      fill=blend(BLUE, a * 0.35), width=2)
...
# ── SCENE 4 · BENEFITS (9 s) ──────────────────────────────────────────
BG_B = grad_bg(c1=BG, c2=BG2)

CARDS = [
    ("85%+", "Success Rate",        TEAL),
    ("3-5",  "Sessions Only",       BLUE),
    ("Fast", "Return to Activity",  PURPLE),
    ("Root", "Cause Treatment",     GOLD),
]
CARD_DESCS = [
    "Patients see significant or complete relief after ESWT",
    "Short 15-20 min in-office visits, 1-2 weeks apart",
    "No lengthy recovery or immobilization needed",
    "Triggers genuine tissue repair, not just pain relief",
]
...
def benefits_frame(t):
    ...
    for i, ((stat, label, color), desc) in enumerate(zip(CARDS, CARD_DESCS)):
        delay = 0.9 + i * 1.2
        a = fade(t, delay, delay + 0.55)
        s = sm(min(1, (t - delay) / 0.55)) if t > delay else 0
        if a <= 0:
            continue
        ...
        scale = 0.8 + 0.2 * s
        sw2, sh2 = int(cw * scale), int(ch * scale)
        cx2 = x1 + cw // 2
        cy2 = y1 + ch // 2
        bx1, by1 = cx2 - sw2//2, cy2 - sh2//2
        bx2, by2 = cx2 + sw2//2, cy2 + sh2//2

        draw.rounded_rectangle([bx1, by1, bx2, by2], radius=18,
                                fill=blend((15, 34, 62), a),
                                outline=blend(color, a * 0.3), width=1)
        draw.rounded_rectangle([bx1, by1, bx2, by1+6], radius=9, fill=blend(color, a))

        sb = draw.textbbox((0, 0), stat, font=f(90, True))
        sw3 = sb[2] - sb[0]
        draw.text((cx2 - sw3//2, by1 + 24), stat, font=f(90, True), fill=blend(color, a))

        lb = draw.textbbox((0, 0), label, font=f(40, True))
        lw = lb[2] - lb[0]
...
def cta_frame(t):
    ...
    if a4 > 0:
        bw = int(700 * sm(min(1, (t - 2.0))))
        bx = W//2 - bw//2
        bh = 100
        by = H//2 + 190
        gc = blend(BLUE, a4 * 0.18, BG3)
        draw.rounded_rectangle([bx-12, by-12, bx+bw+12, by+bh+12], radius=58, fill=gc)
        draw.rounded_rectangle([bx, by, bx+bw, by+bh], radius=48, fill=blend(BLUE, a4, BG3))
        btn_txt = "Talk to Your Doctor Today"
        bb = draw.textbbox((0, 0), btn_txt, font=f(46, True))
        bw2 = bb[2] - bb[0]
        draw.text((W//2 - bw2//2, by + 26), btn_txt, font=f(46, True), fill=blend(WHITE, a4, BLUE))

    if a5 > 0:
        facts = ["+  Non-Invasive", "+  FDA-Cleared", "+  No Surgery", "+  Proven"]
        fw_list = [draw.textbbox((0,0), fa, font=f(34, True))[2] for fa in facts]
        gap = 44
        total = sum(fw_list) + gap * (len(facts) - 1)
        x = (W - total) // 2
        fy2 = H//2 + 370
        for fa, fw in zip(facts, fw_list):
            draw.text((x, fy2), fa, font=f(34, True), fill=blend(TEAL, a5, BG3))
            x += fw + gap
